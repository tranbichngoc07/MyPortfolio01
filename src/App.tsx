...
export default function App() {
  ...
  const [hideEditor, setHideEditor] = useState<boolean>(true); // Default to true for a clean view
  const [showSystemHelp, setShowSystemHelp] = useState<boolean>(true);
  const [isEditMode, setIsEditMode] = useState<boolean>(false);

  // Check query parameters to enable Edit Mode
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const hasEditParam = params.has('edit') || params.has('admin');
    setIsEditMode(hasEditParam);
    if (hasEditParam) {
      setHideEditor(false); // If edit parameter exists, open editor side-by-side by default
    }
  }, []);
